app.controller("productEditCtrl", function ($scope, apiHandler, $rootScope) {
    ...
    $scope.category = $rootScope.productCategory;
    $scope.allColors = [];
    $scope.allSizes = [];
    $scope.newFeature = {};
    $scope.data.features = [];
    ...
    $scope.getColors = () => {
        apiHandler.callGet('color/', (response) => {
            $scope.allColors = response.dataList;
        }, (error) => {
        }, true);
    }
    ...
    $scope.getSizes = () => {
        apiHandler.callGet('size/', (response) => {
            $scope.allSizes = response.dataList;
        }, (error) => {
        }, true);
    }
    $scope.addFeature = () => {
    ...
    $scope.getData = () => {
        apiHandler.callGet("product/info/" + $scope.id, (response) => {
            ...
            $scope.isSelected($scope.selectedColors,"colors");
            $scope.isSelected($scope.selectedSizes,"sizes");
            $scope.fillFeatures();
        }, (error) => {
        }, true);
    }
    ...
    //Changed
    $scope.isSelected = (list, type) => {
        if (type == "colors") {
            for (let i = 0; i < $scope.allColors.length; i++) {
                if (list.some(x => x == $scope.allColors[i].id))
                    document.querySelector("#colors_" + $scope.allColors[i].id).checked=true;
            }
        } else {
            for (let i = 0; i < $scope.allSizes.length; i++) {
                if (list.some(x => x == $scope.allSizes[i].id))
                    document.querySelector("#sizes_" + $scope.allSizes[i].id).checked=true;
            }
        }
    }
    ...
    $scope.onColorChanged = (color) => {
        for (let i = 0; i < $scope.allColors.length; i++) {
            if ($scope.allColors[i].id == color.id) {
                if ($scope.selectedColors.indexOf(color.id) == -1) {
                    $scope.selectedColors.push(color.id);
                } else if ($scope.selectedColors.indexOf(color.id) != -1) {
                    $scope.selectedColors.splice($scope.selectedColors.indexOf(color.id), 1);
                    return;
                }
            }
        }
    ...
    $scope.onSizeChanged = (size) => {
        for (let i = 0; i < $scope.allSizes.length; i++) {
            if ($scope.allSizes[i].id == size.id) {
                if ($scope.selectedSizes.indexOf(size.id) == -1) {
                    $scope.selectedSizes.push(size.id);
                } else if ($scope.selectedSizes.indexOf(size.id) != -1) {
                    for (let j = 0; j < $scope.selectedSizes.length; j++)
                        if ($scope.selectedSizes[j] == size.id) {
                            $scope.selectedSizes.splice(j, 1);
                            return;
                        }
                }
            }
